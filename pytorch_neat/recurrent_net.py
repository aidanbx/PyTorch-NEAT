...
def dense_from_coo(shape, conns, dtype, device):
...
class RecurrentNet():
    def __init__(self, n_inputs, n_hidden, n_outputs,
        ...
        if n_hidden > 0:
            self.hidden_responses = torch.tensor(hidden_responses, dtype=dtype, device=device)
            self.hidden_biases = torch.tensor(hidden_biases, dtype=dtype, device=device)

        self.output_responses = torch.tensor(
            output_responses, dtype=dtype, device=device)
    ...
    def create(genome, config, batch_size=1, activation=sigmoid_activation,
               prune_empty=False, use_current_activs=False, n_internal_steps=1, dtype=torch.float32, device=torch.device('cpu')):
